...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<!-- Квитанция о приеме  -->
	<xsl:template match="Файл[Документ[@КНД='1169001']]" mode="Title">
		<xsl:text>Квитанция о приеме</xsl:text>
	</xsl:template>

	<xsl:template match="Файл[Документ/@КНД='1169001']">
		<xsl:apply-templates select="Документ[@КНД='1169001']"/>
	</xsl:template>

	<xsl:template match="Документ[@КНД='1169001']">
		<p align="right" style="text-align:right">
			<span style="font-size:11.0pt">
				Форма по КНД 1169001
			</span>
		</p>
		...
		<table class="noborder">
			<tr>
				...
				<td style="text-align:center">
					<xsl:for-each select="СвКвит/@ДатаДокНП">
						<xsl:value-of select="."/>
					</xsl:for-each>
				</td>
				<td width="70pt">,</td>
				...
				<td style="text-align:center">
					<xsl:for-each select="СвКвит/@РегНом">
						<xsl:value-of select="."/>
					</xsl:for-each>
				</td>
			</tr>
		...
		<table class='noborder'>
			<tr>
				<td width="450">
					<div>
						который поступил <xsl:value-of select="СвКвит/@ДатаПост"/> и зарегистрирован за Номером
					</div>
				</td>
				<td>
					<div style="text-align:center; margin-right:0pt;">
						<xsl:apply-templates select="СвКвит/@НомерДокНП"/>
					</div>
				</td>
				<td>,</td>
			</tr>
		...
		<xsl:if test="СвКвит/Ошибки">
			<div>Сведения о выявленных ошибках форматно-логического контроля:</div>
			<table cellspacing="0" border="1" width="100%">
				<tr>
					<th width="300">Код ошибки:</th>
					<th>Сообщение об ошибке:</th>
				</tr>
				<xsl:for-each select="СвКвит/Ошибки">
					<tr>
						<td>
